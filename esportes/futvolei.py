...
def iniciar():
    ...
    arquivo = "eventos_futvolei.csv"

    try:
        open(arquivo, "x").write("id,jogador,tipo,zona_propria,zona_adversaria\n")
    except:
        pass
    ...
    def proximo_id():
        with open(arquivo, "r") as f:
            linhas = f.readlines()
            return len(linhas)  # ignora header automaticamente


    def selecionar_propria(zona):
        nonlocal zona_propria
        zona_propria = zona
    ...
    def registrar():
        ...
        with open(arquivo, "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([id_evento, jogador, tipo, zona_propria, zona_adv])

        status["text"] = f"Registrado #{id_evento}: {jogador} | {tipo}"


    root = tk.Tk()
    root.title("Tagger Futevôlei")
    ...
    jogador["values"] = ["Chau", "Alberto", "J3", "J4"]
